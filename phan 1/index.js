const productDetails = [
    {
        ...
        frontCamera: "7 MP",
        imageUrl: "https://cdn.tgdd.vn/Products/Images/42/114115/iphone-x-64gb-hh-600x600.jpg",
        qty: 10, 
    ...
    },
    {
        ...
        frontCamera: " 32 MP",
        imageUrl: "https://cdn.tgdd.vn/Products/Images/42/217536/samsung-galaxy-m51-trang-new-600x600-600x600.jpg",
        qty: 10,
    ...
    },
    {
        ...
        backCamera: "Chính 12 MP & Phụ 64 MP, 12 MP",
        frontCamera: "32 MP",
        imageUrl: "https://didongviet.vn/pub/media/catalog/product//i/p/iphone-11-pro-max-256gb-didongviet_23.jpg",
        qty: 10,
        desc: "Thiết kế đột phá, màn hình tuyệt đỉnh",
        type: "Iphone"
    },
  ...
  function okay(event) {
    ...
    if (event.target.innerText == "Tiếp tục") {
      ...
      event.target.parentElement.getElementsByClassName(
        "order-details"
      )[0].innerHTML = `<em class='thanks'>Cảm ơn vì đã đặt hàng</em>`;
  ...
  function Product(product = {}) {
    let { name, price, screen, backCamera, frontCamera, imageUrl, type, desc } = product;
    return `
  <div class='card'>
    <div class='top-bar'>
      <em class="stocks">Còn hàng</em>
    </div>
    <div class='img-container'>
      <img class='product-img' src='${imageUrl}' alt='' />
      <div class='out-of-stock-cover'><span>Hết hàng</span></div>
    </div>
    <div class='details'>
      <div class='name-fav'>
        <strong class='product-name'>${name}</strong>
        <button onclick='this.classList.toggle("fav")'</button>
      </div>
      <div class='wrapper'>
        <h5>Loại: ${type}</h5>
        <p>Mô tả: ${desc}</p>
  ...
  <div class='side-nav'>
    ...
    <div class='final'>
      <strong>Tổng tiền: <span class='total'>0</span></strong>
      <div class='action'>
        <button onclick='buy(1)' class='btn buy'>Mua <i class='fas fa-credit-card' style='color:#6665dd;'></i></button>
        <button onclick='clearCart()' class='btn clear'>Xóa <i class='fas fa-trash' style='color:#bb342f;'></i></button>
      </div>
    </div>
  </div>`;
  }
  ...
  function Purchase() {
    ...
    let itemPrices = cartDetails.map((item) => {
      return `<span>Giá: ${item.price * item.qty}</span>`;
    });
    return `
  <div class='invoice'>
  ...
  <hr>
    <div class='payment'>
      <em>Thanh toán</em>
      <div>
        <p>Tổng số tiền thanh toán:</p><span class='pay'>Giá: ${toPay}</span>
      </div>
    </div>
    <div class='order'>
      <button onclick='order()' class='btn-order btn'>Đặt hàng</button>
  ...
  <div>
    <div class='order-details'>
      <em>Đơn hàng đã hoàn tất</em>
      <p>Mã đơn hàng: <span>${orderId}</span></p>
      <p>Đơn hàng sẽ được giao từ 3 - 5 ngày sau khi đặt</p>
      <p>Quý khách có thể thanh toán <span>SỐ TIỀN ${totalCost}</span> bằng tiền mặt hoặc chuyển khoản khi nhận hàng</p>
    </div>
  ...
  function CartIsEmpty() {
    let emptyCart = `<span class='empty-cart'>Vui lòng chọn sản phẩm</span>`;
